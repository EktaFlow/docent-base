...
describe("determine MR Level is same as Target MRL", function() {
  ...
  it("clicks Start New", () => {
    ...
    cy.get(
      ".number-info > span:nth-child(1)"
      ).should($s => {
        expect($s).to.contain.text(
          `MR Level: ${value}`
          );
          mrLevel = $s.text();
        });
    ...
    cy.get(
      ".targets-scope > p:nth-child(1)"
      ).should($p => {
        expect($p).to.contain.text(
          `Target MRL: ${value}`
          );
          ...
          mrLevel = mrLevel.slice(-1);
          targetMrl = targetMrl.slice(-1);
          // console.log(mrLevel);
