...
export class ThreadsListComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute) {
    ...
    this.threadsSelected = this.activatedRoute.snapshot.paramMap.get(
      "threadsSelected"
    );
  }
  ...
  emit(item) {
    ...
    if (this.threadsSelected.includes(item.index)) {
      this.threadsSelected = this.threadsSelected.filter(
        (a) => a !== item.index
      );
    } else {
      this.threadsSelected.push(item.index);
    }
  }
}
