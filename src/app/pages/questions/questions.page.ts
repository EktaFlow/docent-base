...
export class QuestionsPage implements OnInit {
  ...
  async updateAssessment(values) {
    ...
    tempAssessmentObject.splice(
      this.currentQuestion.questionId - 1,
      1,
      newerQuestion
    );
    this.allQuestions = tempAssessmentObject;
    this.currentQuestion = this.allQuestions.filter(
    ...
    if (hasOfflineAnswers) {
      var saveOffline = await this.assessmentService.updateQuestionSeries(
        hasOfflineAnswers
      );
      if (saveOffline) {
    ...
    update.subscribe((data) => {
      ...
      if ((<any>data).data) {
        ...
        this.storage.remove("offline").then((p) => {
          p
            ? console.log("something was removed")
            : console.log("nothing removed... nothing exist??");
        });
      }
  ...
  handleLevelSwitching() {
    ...
    // make sure this encompasses all non-switching scenarios.
      ...
      alert(
        `You have failed this subthread, you will be shown questions from this subthread at the next lowest level ${mrLevel}`
      );
  ...
  threadAnswered() {
    ...
    return this.allQuestions
      .filter((q) => q.mrLevel == mrLevel && q.subThreadName == subThreadName)
  ...
  hasFloorLevel() {
    ...
    currentSubthread.forEach((q) => {
      ...
      level.every((ques) =>
        ["Yes", "Skipped", "skipped", "N/A"].includes(ques.currentAnswer)
      )
        ? (floor = true)
        : null;
    });
  ...
  addLowerMRL() {
    ...
    var newSurvey = [...nextLowest, ...this.surveyQuestions].sort(
      (a, b) => a - b
    );
  ...
  public findAmtOfQs() {
    ...
    this.currentQPos =
      this.surveyQuestions.indexOf(this.currentQuestion.questionId) + 1;
  }
  ...
  public calculateRiskScore() {
    ...
    if (likelihood && consequence) {
      (<any>document.querySelectorAll(".matrix-row th")).forEach((element) => {
        element.className = element.className.replace(/selected/g, "");
        element.innerHTML = "";
      });
